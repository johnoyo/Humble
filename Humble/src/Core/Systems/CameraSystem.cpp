...
namespace HBL {
	...
	{
		glm::mat4 tranform =
			glm::translate(glm::mat4(1.0f), Globals::s_Registry.GetComponent<Component::Transform>(camera).position) *
	...
	{
		Set_Position_x(camera, Globals::s_Registry.GetComponent<Component::Transform>(player).position.x + offset);
		Set_Position_y(camera, Globals::s_Registry.GetComponent<Component::Transform>(player).position.y + offset);
		Recalculate_View_Matrix(camera);
	}

	void CameraSystem::Follow(IEntity& camera, IEntity& player, float offset_x, float offset_y)
	{
		Set_Position_x(camera, Globals::s_Registry.GetComponent<Component::Transform>(player).position.x + offset_x);
		Set_Position_y(camera, Globals::s_Registry.GetComponent<Component::Transform>(player).position.y + offset_y);
		Recalculate_View_Matrix(camera);
	}
