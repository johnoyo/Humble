...
void HBL::Level0::OnAttach()
{
	...
	Registry::Get().GetComponent<Component::Tag>(wall[8]).tag = "PinkTile";
	...
	for (uint32_t i = 0; i < 10000; i++)
	{
		...
		Registry::Get().GetComponent<Component::Transform>(level[i]).position.y = Random::Float(400.0f, 800.0f);
	}

	Registry::Get().GetComponent<Component::Text>(text).scale.x = 2.0f;
	Registry::Get().GetComponent<Component::Text>(text).scale.y = 2.0f;
	Registry::Get().GetComponent<Component::Text>(text).text = "aaaaaaaaaaaaaaaaaaaaaaaaaaa";

	Registry::Get().GetComponent<Component::Text>(FPSCounter).position.x = 500.0f;
	Registry::Get().GetComponent<Component::Text>(FPSCounter).position.y = 500.0f;

	Registry::Get().GetComponent<Component::Text>(FPSCounter).sreenSpaceOffset.x = 100.0f;
	Registry::Get().GetComponent<Component::Text>(FPSCounter).sreenSpaceOffset.y = 600.0f;

	Registry::Get().GetComponent<Component::Text>(FPSCounter).scale.x = 1.0f;
	Registry::Get().GetComponent<Component::Text>(FPSCounter).scale.y = 1.0f;

	Registry::Get().GetComponent<Component::Text>(FPSCounter).screenSpace = true;
